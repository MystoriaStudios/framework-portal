...
CREATE
POLICY select_all_policy ON public.private_items FOR
SELECT USING (TRUE);

-- 3. Create Policy for INSERT
CREATE
POLICY insert_auth_policy ON public.private_items FOR
INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- 4. Create Policy for UPDATE
CREATE
POLICY update_auth_policy ON public.private_items FOR
