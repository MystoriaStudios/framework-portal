      ...
      <div className="mt-12 transition-all delay-300 transition">
        {!isValidating && data && data.length > 0 ? (
            ...
            >
              {data
                ...
                .map((node: any) => {
                  ...
                  const state =
                    node.state != 'OFFLINE' && date > 60 * 1000
                      ? 'CRASHED'
                      : node.state;
                    ...
                    >
                      <Link
                        href={
                          state === 'OFFLINE'
                            ? '/dashboard'
                            : `/dashboard/nodes/${node.identifier}`
                        }
                      ...
                      >
                        ...
                        <span className="text-mds my-auto col-span-5 flex flex-col ml-2 tracking-widest mr-auto border-black">
                          ...
                          <span className="-mt-1 text-sm w-56 dark:text-neutral-300">
                            ...
                            {date < 2000
                              ? 'just now'
                              : date > 60 * 1000
                              ? (date / 60000).toString().split('.')[0] + 'm'
                              : (date / 1000).toString().split('.')[0] +
                                's'}{' '}
                            {date > 2000 ? 'ago' : ''}
                          </span>
                        </span>
          ...
          <div className="text-neutral-50 px-8 pb-5 text-2xl text-center transition-all delay-300">
            There are no nodes found on this organization you can follow our
            node setup guide{' '}
